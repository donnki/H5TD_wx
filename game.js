...
if (!GameGlobal.Utils) GameGlobal.Utils = {};
var Utils = GameGlobal.Utils;
...
Utils.addMobileListeners = function(landscape, ignoreIOS7) {
    ...
    if(landscape){
        // setInterval(Utils.checkOrientation(true), 500);
        setInterval(()=>{
            Utils.checkOrientation(true)
        }, 500);
    }else{
        // setInterval(Utils.checkOrientation(false), 500);
        setInterval(()=>{
...
Utils.alignElement = function(s, windowRect, width, height) {
    ...
    else s.style.left = Math.floor((windowRect.width - width - Utils.layoutMargin.left - Utils.layoutMargin.right) / 2) + "px"; if (Utils.fitLayoutVerticalAlign == "top") s.style.top = Utils.layoutMargin.top + "px";
...
Utils.ajax = function(url, method, params, dataType, successCallback, failCallback) {
    ...
    else xmlhttp = new ActiveXObject("Microsoft.XMLHTTP"); if (Utils.isIE() && window.XDomainRequest) {
    ...
    } else params = ""; if (!method) method = "GET";
...
AssetsLibrary.prototype.getSprite = function(name, params, spriteClass) {
    ...
    if (spriteClass.create && typeof spriteClass.create == "function") mc = spriteClass.create(asset, this);
        ...
        asset.width, asset.height, asset.frames, asset.layers); if (params && typeof params == "object")
...
Tween.prototype.tick = function(delta) {
    ...
    else this._pos += delta; if (this._pos < 0) return false;
            ...
            if (!this.hasEventListener("prerender") || this.dispatchEvent("prerender", {
                ...
                if (this.mask) {
                    ...
                    if (this.mask.render) this.mask.render(this.stage.buffer, drawStatic, delta, true);
                        ...
                        this.mask.x ? this.mask.x : 0, this.mask.y ? this.mask.y : 0); if (Sprite.FLOOR_VALUES_ON_RENDER) cns.ctx.drawImage(this.stage.buffer, 0, 0, rect.width, rect.height, -Math.floor(ow / 2), -Math.floor(oh / 2), ~~ow, ~~oh);
...
StageTimer.prototype.update = function(delta) {
    ...
    else this.timeout -= delta; if (this.timeout <= 0) {
...
Stage.prototype.drawLine = function(x1, y1, x2, y2, width, color, opacity, ignoreViewport) {
    ...
    else cns.ctx.strokeStyle = "#000"; if (width) cns.ctx.lineWidth = width * Utils.globalScale;
    else cns.ctx.lineWidth = 1 * Utils.globalScale; if (opacity) cns.ctx.globalAlpha = opacity;
...
var ExternalAPI = {
    ...
    showAds: function() {/*
        if (ExternalAPI.isPortalEnvironment())
            if (window.Advertizing && window.Advertizing.show) Advertizing.show()*/
    },
...
var TTLoader = {
    ...
    create: function(callback, landscape, skipButton, disableLogoLink) {
        ...
        //     Utils.bindEvent(a, event, function() {
    ...
    setSizes: function() {
        ...
        //     Math.floor(c * 80) + "px";
        // document.getElementById("tt_load_logo").setAttribute("width", Math.floor(c * 300) + "px");
        // document.getElementById("tt_load_logo").setAttribute("height", Math.floor(c * 65) + "px");
...
BitmapText.prototype.manageSprites = function(text) {
    ...
    if (sp_len < len)
        for (i = 0; i < len - sp_len; i++) {
            chr = new GameGlobal[BitmapText.spriteClass](null, 0, 0);
...
BitmapText.prototype.write = function(text) {
    ...
    for (var i = 0; i < this.lines.length; i++) {
        ...
        if (this.align ==
            BitmapText.ALIGN_RIGHT) curX = this.x - this.getWidthOfLine(i) * this.scale;
...
AudioPlayer.prototype.readyToPlay = function(e) {
    if (!e.currentTarget || e.currentTarget.alreadyLoaded) return;
...
ParticleSystem.Utils.extend = function(obj, config) {
    for (var prop in config)
        if (config.hasOwnProperty(prop)) obj[prop] = config[prop]
};
ParticleSystem.Utils.recursiveExtend = function(obj, config, exceptions) {
    exceptions = exceptions || [];
    for (var prop in config)
        if (config.hasOwnProperty(prop))
            if (exceptions.indexOf(prop) > -1) obj[prop] = config[prop];
...
ParticleSystem.Utils.deepClone = function(obj, exceptions) {
    ...
    var clone = {};
    for (var prop in obj)
        if (exceptions.indexOf(prop) > -1) clone[prop] = obj[prop];
...
ParticleSystem.Emitter = function(system, position, displayObjects) {
    ...
    else this.displayObjects = Utils.isArray(displayObjects) ? displayObjects : [displayObjects]; if (!system || typeof system === "string") {
...
ParticleSystem.Emitter.prototype.reconfigure = function(config) {
    ...
    if (!this.emissionDuration) this.emissionDuration =
        Infinity;
...
ParticleSystem.Emitter.prototype.setStatic = function(val) {
    ...
    for (var i = 0; i < this._particlePool.length; i++)
        if (this._particlePool[i].displayObject) this._particlePool[i].displayObject.setStatic(val)
};
ParticleSystem.Emitter.prototype._initParticle = function(particle) {
    ...
    if (this.posVarTransformFn) posVar = this.posVarTransformFn(posVar,
        ParticleSystem.Utils);
    ...
    var life = this.life + this.lifeVar * ParticleSystem.Utils.random11() ||
        0;
    ...
    particle.radius =
        ParticleSystem.Utils.isNumber(this.radius) ? this.radius + (this.radiusVar || 0) * ParticleSystem.Utils.random11() : 0;
    ...
    particle.rotationSpeed += ParticleSystem.Utils.isNumber(this.rotationSpeedVar) ? this.rotationSpeedVar *
        ParticleSystem.Utils.random11() : 0;
    ...
    if (this.startColor) {
        var startColor = [this.startColor[0] + this.startColorVar[0] * ParticleSystem.Utils.random11(), this.startColor[1] + this.startColorVar[1] * ParticleSystem.Utils.random11(), this.startColor[2] + this.startColorVar[2] * ParticleSystem.Utils.random11(),
            this.startColor[3] + this.startColorVar[3] * ParticleSystem.Utils.random11()
        ];
        ...
        particle.deltaColor = [(endColor[0] - startColor[0]) / particle.life, (endColor[1] -
            startColor[1]) / particle.life, (endColor[2] - startColor[2]) / particle.life, (endColor[3] - startColor[3]) / particle.life]
    }
...
ParticleSystem.Emitter.prototype.clear = function() {
    for (var i = 0; i < this._particlePool.length; i++)
        if (this._particlePool[i].displayObject) this._particlePool[i].displayObject.destroy = true;
    this._particlePool = [];
    this._particleCount = 0;
    this._particleIndex = 0;
    this._elapsed = 0;
...
ParticleSystem.Emitter.prototype.update = function(delta) {
    ...
    if (this.path) {
        ...
        if (this._pathStartDelay <= 0) {
            ...
            if (this._pathPassedTime > this.path.duration)
                if (this.path.loop) this._pathPassedTime -=
                    Math.floor(this._pathPassedTime / this.path.duration) * this.path.duration;
                else this._pathPassedTime = this.path.duration;
            if (typeof this.path._ease == "string")
                if (this.path._ease == "null") this.path._ease = null;
                else {
            ...
            var len = f(this._pathPassedTime, 0, this._pathLen,
                this.path.duration);
        ...
        if (p.life > 0 && (p.color || p.displayObject)) {
            if (p.displayObject) {
                ...
                if (p.displayObject instanceof Graphics) {
                    ...
                    p.displayObject.fillColor =
                        p.displayObject.color
                }
...
DataCache.load = function(url, callback, dataType, params, method) {
    ...
    Utils.ajax(url, method, params, dataType, function(data) {
...
DataCache.batch = function(batch, completeCallback) {
    ...
    for (var i = 0; i < itemsCount; i++) {
        item = batch[i];
        DataCache.load(item.url, function(cb) {
    ...
    if (size >= 4) {
        for (var idx = 0; idx < size - 3; idx++)
            for (var t = 0; t <= 1; t += .05) {
                ...
                if (s == 0 || (Math.abs(this.m_path[s - 1].x - pos.x) >= .5 ||
                    Math.abs(this.m_path[s - 1].y - pos.y) >= .5)) this.m_path.push(pos)
            }
        return true
    }
...
TD.getWorldConfig = function(id) {
    ...
    for (var i = 0; i < worlds.length; i++)
        if (TD.getAttr(worlds[i], "world_number") == id) return worlds[i];
    return null
};
TD.startLevel = function(worldId, id) {
    //ExternalAPI.exec("showAds"); //edits
    TD.clear();
    ...
    var batchConfig = [{
        ...
        url: HTTP_JSON_SERVER + "data/game/towers.json",
        callback: function(data) {
            TD.configs.towers = data
        }
    }, {
        url: HTTP_JSON_SERVER + "data/game/towers_bar.json",
        callback: function(data) {
            TD.configs.towersBar = data
        }
    }, {
        url: HTTP_JSON_SERVER + "data/game/bullets.json",
        callback: function(data) {
            TD.configs.bullets = data
        }
    }];
    DataCache.batch(batchConfig, function() {
...
TD.createUI = function() {
    ...
    TD.gameField.addEventListener("wavefinished",
        TD.clearWaveIcons);
    ...
    TD.ui.optionsButton.addEventListener("click",
        SoundsManager.click);
...
TD.clearWaveIcons = function() {
    if (TD.ui.waveIcons)
        for (var i = 0; i < TD.ui.waveIcons.length; i++) TD.ui.waveIcons[i].destroy = true;
    TD.ui.waveIcons = []
};
...
TD.buyAdditionalSaveSlot = function() {
    if (TD.gameData.stars >= TD.ADDITIONAL_SLOT_PRICE) {
        ...
        Alert.create(text, TD.processBuyAdditionalSaveSlot,
            true)
...
TD.load = function(callback) {
    if (ExternalAPI.externalStorage) ExternalAPI.exec("load", function(data) {
...
TD.updateTowerMenu = function() {
    if (TD.ui.towerMenu)
        if (TD.ui.towerMenu.tower.health <= 0 || !TD.ui.towerMenu.tower.parent) TD.hideTowerMenu();
...
TD.tick = function(delta) {
    if (TD.gameField && TD.gameField.ready) {
        ...
        else TD.ui.moveyView.animatedNumericUpdate(TD.gameField.money); if (TD.ui.lifeView.text == "") TD.ui.lifeView.update(TD.gameField.life);
...
TD.getPerk = function(name) {
    ...
    for (var i = 0; i < perks.length; i++)
        if (TD.getAttr(perks[i], "name") == name) return perks[i];
    return null
};
TD.metrics = {};
TD.metrics.scale = .5;
...
GameField.prototype.init = function(mission, unitsConfigs) {
    ...
    Mob.reset();
    for (var i in unitsConfigs.units)
        if (i != "comment") Mob.typeNames.push(i);
    var preloader = new ImagesPreloader;
    this.gameViews = {};
    this.gameViews.back = this.getImage(this.mission.background.background);
    var base = this.mission.background.base ? this.mission.background.base :
        this.mission.background.bases.base_01;
...
GameField.prototype.buildPathes = function() {
    ...
    for (var i = 0; i < sep.length; i++) this.separator.addPoint(TD.parsePositionString(TD.getAttr(sep[i],
        "point")));
...
GameField.prototype.getPathById = function(id) {
    for (var i = 0; i < this.pathes.length; i++)
        if (this.pathes[i].m_id == id) return this.pathes[i];
    return null
};
...
GameField.prototype.highLightSpots = function(val) {
    ...
    for (var i = 0; i < this.spots.length; i++)
        if (this.spots[i].state != Spot.STATE_FAILED && !this.spots[i].tower) this.spots[i].setState(state)
};
...
GameField.prototype.tick = function(delta) {
    ...
    if (this.tickWave(delta)) {
        ...
        if (!this.mobs.length && !this.mobsPortals.length)
            if (!this.startNextWave()) {
                this.win();
                return
            }
    }
...
GameField.prototype.debugDraw = function() {
    ...
    for (var n = 0; n < this.pathes.length; n++) {
        ...
        for (var i = 0; i < this.pathes[n].m_base.length; i++) stage.drawCircle(this.x + this.pathes[n].m_base[i].x, this.y + this.pathes[n].m_base[i].y, 2,
            1, colors[n])
    }
    if (this.separator) {
...
function Mob(missionConfig, unitConfig, path) {
    ...
    this.selfRadius = TD.getAttr(unitConfig, "self_radius",
        true) * TD.metrics.scale;
    ...
    TD.setHotSpot(this.baseView, TD.getAttr(unitConfig.base,
        "hotspot"));
    this.shadow = null;
    if (this.aircraft) {
        ...
        this.shadow.changeFrameDelay =
            1E3 / TD.getAttr(unitConfig.base, "fps", true);
    ...
    if (unitConfig.animations) {
        ...
        for (var i = 0; i < unitConfig.animations.animation.length; i++) {
            ...
            animSprite.changeFrameDelay = 1E3 / TD.getAttr(animation,
                "fps", true);
...
Mob.prototype.tick = function(delta, speedScale) {
    ...
    this.distance += this.getWalkStepLength(delta *
        sm);
...
function Cannon(config) {
    ...
    this.attackRadius = TD.getAttr(config,
        "attack_radius", true) * TD.metrics.scale;
...
Cannon.prototype.findTarget = function() {
    ...
    var exclude = [];
    for (var i = 0; i < mob.cannons.length; i++)
        if (mob.cannons[i] != this && mob.cannons[i].target) exclude.push(mob.cannons[i].target);
...
Cannon.prototype.tick = function(delta, speedScale) {
    ...
    var rl = diff < 0 ? -diff : this.rotation + Math.PI * 2 -
        angle;
    ...
    this.rotation += step;
    if (this.target && this.canShoot && this.rotation == angle && this.attackDelay <= 0)
        if (this.shootState) {
...
Tower.prototype.reconfigure = function() {
    ...
    var angle = 0,
        gun, gunsList = Utils.isArray(this.config.baseConfig.guns.gun) ?
        this.config.baseConfig.guns.gun : [this.config.baseConfig.guns.gun];
...
Tower.prototype.stop = function() {
    ...
    for (var i = 0; i < this.guns.length; i++)
        if (this.guns[i].flameView) this.guns[i].flameView.visible = false
};
...
Tower.prototype.getNextLevelProps = function() {
    ...
    if (this.subLevel < 2)
        if (this.isUpOpened(level, subLevel + 1)) return [{
            level: level,
            subLevel: subLevel + 1
        }];
        else return [];
...
Gun.prototype.findTarget = function() {
    ...
    for (var i = 0; i < this.parent.guns.length; i++)
        if (this.parent.guns[i] != this && this.parent.guns[i].target) usedTargets.push(this.parent.guns[i].target);
    var target = null,
        maxLenPath = -1;
    for (var i =
        0; i < targetsInRadius.length; i++) {
...
Gun.prototype.tick = function(delta, speedScale) {
    ...
    if (this.target) {
        ...
        this.rotation = TD.normalizeAngle(this.rotation);
        var diff = angle - this.rotation;
        var rr = diff > 0 ? diff : Math.PI * 2 - this.rotation +
            angle;
        ...
        if (this.rotation == angle && this.attackDelay <= 0) {
            ...
            if (this.flamethrower) this.flameView.visible = true;
                ...
                this.fireShotView.rotation = Math.PI /
                    2;
...
TowerPlace.prototype.checkPlace = function() {
    ...
    if (this.config.forSave) {
        var buttons = TD.ui.savedTowersBar.objects;
        for (var i = 0; i < buttons.length; i++)
            if (buttons[i].config.towerId < 0 && buttons[i].config.opened && this.pointer.hitTest(buttons[i])) {
                this.valid = true;
        ...
        for (var i = 0; i < spots.length; i++)
            if (spots[i].state != Spot.STATE_FAILED && !spots[i].tower && this.pointer.hitTest(spots[i])) {
                this.valid = true;
                this.selectedTarget =
                    spots[i];
                break
...
function Bullet(id, hitCoefficient, subLevel) {
    ...
    this.toX = 0;
    this.toY = 0;
    this.speed = TD.getValFromString(TD.getAttr(this.config,
        "speed"), this.subLevel);
...
Bullet.prototype.tick = function(delta, speedScale) {
    ...
    this.setPosition(this.x +
        Math.cos(this.pathAngle) * len, this.y + Math.sin(this.pathAngle) * len);
...
Bullet.getConfigById = function(id) {
    for (var i in TD.configs.bullets)
        if (i == id) return TD.configs.bullets[i];
...
function TowersHierarchy(parent, config) {
    ...
    this.price = TD.parseNumbersString(TD.getAttr(config,
        "price"));
    ...
    this.slowUnitId = TD.parseNumbersString(TD.getAttr(config,
        "slow_unit_id"));
    ...
    for (var i in config)
        if (i != "@attributes" && i != "comment") this.children.push(new TowersHierarchy(this, config[i]))
}
TowersHierarchy.prototype.getSlowEffect = function(subLevel, unitId) {
    if (this.slowTime[subLevel] > 0 && this.slowUnitId[unitId])
        if (Math.random() <= this.slowProbability[subLevel]) return {
...
UpgradesManager.checkComplete = function(id, level) {
    ...
    for (var i = 0; i < config.children.length; i++)
        if (!UpgradesManager.checkComplete(id, config.children[i].level)) return false;
    return true
};
...
AchievementsManager.ALL_ACHIEVEMENTS = [AchievementsManager.ENEMY_WILL_BE_HELD, AchievementsManager.GREEN_BERET, AchievementsManager.CONQUEST_OF_SNOW, AchievementsManager.WAR_IN_THE_SANDBOX, AchievementsManager.TOTAL_SWEEP, AchievementsManager.UNDER_THE_COVER_OF_SNOWFALL, AchievementsManager.STORM_IN_THE_SANDBOX, AchievementsManager.ELITE_GUNNERS, AchievementsManager.ELITE_FLAMETHROWER, AchievementsManager.ELITE_ARTILLERY, AchievementsManager.ELITE_ANTIAIRCRAFT_GUNNERS, AchievementsManager.INCREDIBLE_ARMY, AchievementsManager.SOLDIERS_AFTER,
    AchievementsManager.BIG_BARRELS, AchievementsManager.NO_CHANCE_TO_THE_ENEMY, AchievementsManager.PERFECTION
];
...
AchievementsManager.event = function(type, val) {
    ...
    if (type == AchievementsManager.EVENT_LEVEL_COMPLETE) {
        ...
        for (var i = 0; i < 4; i++)
            for (var n = 0; n < TD.gameData.levelResults[i].length; n++) {
                completedLevels[i]++;
                if (TD.gameData.levelResults[i][n] && TD.gameData.levelResults[i][n].award >= 2) maxStartOnLevels[i]++
            }
        if (completedLevels[0] >=
            TD.LEVELS_ON_CHAPTER) AchievementsManager.complete(AchievementsManager.GREEN_BERET);
    ...
    if (type ==
        AchievementsManager.EVENT_MOB_KILL) {
    ...
    for (var i = 0; i < AchievementsManager.ALL_ACHIEVEMENTS.length; i++)
        if (AchievementsManager.ALL_ACHIEVEMENTS[i] != AchievementsManager.PERFECTION) {
...
SoundsManager.getSoundConfig = function(name) {
    for (var i = 0; i < SoundsManager.config.length; i++)
        if (TD.getAttr(SoundsManager.config[i], "name") == name) return SoundsManager.config[i]
};
...
SoundsManager.play = function(name, isMusic) {
    ...
    if (config) {
        ...
        if (maxCount > 0) {
            var cnt = 0;
            for (var i = 0; i < SoundsManager.mixer.channels.length; i++)
                if (SoundsManager.mixer.channels[i].busy && SoundsManager.mixer.channels[i].playedFileName == fileName) cnt++;
...
Alert.prototype.createChildren = function() {
    ...
    txt.setScale(.6);
    this.addChild(txt);
    var yesButton = new CommonButton("button_confirm_no",
        "ico_yes_red");
...
TowerButton.prototype.refresh = function() {
    if (this.config.towerId < 0) {
        ...
        this.config.cost =
            TowersHierarchy.get(this.config.towerId, 0).getCost(0);
...
function TowerMenu(tower) {
    ...
    this.buttons = [];
    this.addButton(new TowerMenuSellButton(tower),
        Math.PI / 4 * 3, Utils.proxy(this.sell, this));
...
MainMenu.prototype.createChildren = function() {
    ...
    this.addChild(b);
    b = new CommonButton("button_medium_square",
        "achievements");
    b.setPosition(100, 150);
...
ChapterSelect.prototype.createChildren = function() {
    ...
    for (var i = 0; i < worlds.length; i++) {
        world = worlds[i];
        if (ChapterSelect.worldsToDisplay.indexOf(TD.getAttr(world, "world_number", true)) >= 0) 
        displayWorlds.push(world)
    }
    var lastOpened = -1;
    for (i = 0; i < displayWorlds.length; i++) {
        var icon =
            new ChapterIcon(displayWorlds[i]);
...
ChapterIcon.prototype.displayStars = function(data) {
   ...
   // var gainedStars = 0,//edit by jiazom
   var gainedStars = 0,
        results = TD.gameData.levelResults[this.worldId];
    if (results)
        for (i = 0; i < results.length; i++)
            if (results[i]) gainedStars += results[i].stars;
...
LevelSelect.prototype.showWorldUI = function(data) {
    ...
    for (var i = 0; i < data.mission.length; i++) {
        ...
        totalStars +=
            TD.getValFromString(TD.getAttr(data.mission[i], "money_finish"), 0)
    }
...
LevelWin.prototype.createChildren = function() {
    ...
    var worldScore = 0;
    for (var i = 0; i < worldResults.length; i++)
        if (worldResults[i]) worldScore +=
            worldResults[i].score;
...
LevelWin.prototype.animateVals = function() {
    ...
    else SoundsManager.play("next_mission_count_effect"); if (this.currentAnimatedVal >= this.valsToAnimate.length) return;
...
Notification.prototype.loadConfig = function() {
    DataCache.load(HTTP_JSON_SERVER + "data/game/notifications.json", this.createChildren)
};
Notification.prototype.createChildren = function(config) {
    this.setPosition(this.stage.screenWidth / 2, -25);
    if (typeof this.id == "number")
        for (var i = 0; i < config.achievements.a.length; i++)
            if (TD.getAttr(config.achievements.a[i], "id", true) == this.id) {
                this.id = TD.getAttr(config.achievements.a[i], "name");
                break
            }
    var data = null;
    for (var i = 0; i < config.items.item.length; i++)
        if (TD.getAttr(config.items.item[i], "name") == this.id) {
            data = config.items.item[i];
            break
        }
    if (data) {
        if (data.image) {
            var icon = UI.getSpriteByTexture(TD.getAttr(data.image,
                "texture"));
...
Shop.prototype.loadConfig = function() {
    DataCache.load(HTTP_JSON_SERVER + "data/shop/shop.json", Utils.proxy(this.configLoaded, this))
};
...
function ShopTowerButton(config, towerId, towerLevel) {
    ...
    if (config.arrow) {
        this.arrow =
            UI.getSpriteByTexture(TD.getAttr(config.arrow, "texture"));
...
ShopPanel.prototype.update = function(source) {
    ...
    this.stars = new UITextGold2(70,
        86, "\u00a9" + TD.gameData.stars);
...
Achievements.prototype.getConfigById = function(config, id) {
    for (var i = 0; i < config.achievements.a.length; i++)
        if (TD.getAttr(config.achievements.a[i], "id", true) == id) {
            id = TD.getAttr(config.achievements.a[i], "name");
            break
        }
    for (var i = 0; i < config.items.item.length; i++)
        if (TD.getAttr(config.items.item[i], "name") == id) return config.items.item[i]
};
Achievements.prototype.createChildren = function(config) {
    ...
    closeBtn.onclick = Utils.proxy(this.close, this);
    this.addChild(closeBtn);
    var text = new UITextGold2(0, -92, I18.f("\u041d\u0410\u0413\u0420\u0410\u0414\u042b"),
        true);
    this.addChild(text);
    ...
    this.itemsView = new Sprite(holder.cacheAsBitmap(),
        width, height);
...
function AchievementItem(config, completed) {
    ...
    if (config) {
        ...
        if (config.message) {
            var text = new UITextColor(-140, 3, I18.f(TD.getAttr(config.message, "string")),
                completed ? UITextColor.defaultColor : "#aaa");
            this.addChild(text);
...
NewGame.prototype.createChildren = function() {
    ...
    this.addChild(back);
    var txt = new UITextColor(0, -52, I18.f("\u041d\u0430\u0447\u0430\u0442\u044c \u0437\u0430\u043d\u043e\u0432\u043e? \u0412\u0435\u0441\u044c \u043f\u0440\u043e\u0433\u0440\u0435\u0441\u0441 \u0431\u0443\u0434\u0435\u0442 \u0443\u0442\u0435\u0440\u044f\u043d."),
        UITextColor.defaultColor, true);
    txt.view.maxWidth = 280;
    txt.view.valign = BitmapText.VALIGN_MIDDLE;
    txt.setScale(.7);
    this.addChild(txt);
...
Hint.show = function(id, obj, arrowPos) {
    ...
    DataCache.load(HTTP_JSON_SERVER + "data/game/tooltip.json", function(data) {
...
UIText.prototype.animatedNumericUpdate = function(val, finishCallback) {
    ...
    this.animateTween.onfinish =
        this.updateOnTweenFinish;
...
var I18 = {
    ...
    init: function(locale, callback) {
        ...
        Utils.get(HTTP_JSON_SERVER + "language/" + locale + ".json?v=" + (new Date).getTime(), null, "json", function(data) {
            var strings = {};
            var parts = data.s;
            for (var i = 0; i < parts.length; i++) strings[I18.trim(parts[i].text)] =
                I18.trim(parts[i].translation);
            I18.setup(strings);
            if (callback) callback()
        })
    },
    ...
    sprintf: function() {
        ...
        var doFormat = function(substring, valueIndex, flags, minWidth, _, precision, type) {
            ...
            switch (type) {
                ...
                case "u":
                    return formatBaseX(value, 10,
                        prefixBaseX, leftJustify, minWidth, precision, zeroPad);
                case "i":
                case "d":
                    number = +value || 0;
                ...
                case "G":
                    ...
                    textTransform = ["toString", "toUpperCase"]["eEfFgG".indexOf(type) %
                        2
                    ];
...
function loadLocalizationEnd() {
    var batchConfig = [{
        url: HTTP_JSON_SERVER + "data/game/towers_hierarchy.json",
        callback: TowersHierarchy.load
    }, {
        url: HTTP_JSON_SERVER + "data/game/perks.json",
        callback: function(data) {
            TD.configs.perks = data
        }
    }, {
        url: HTTP_JSON_SERVER + "data/game/soundmanager.json",
        callback: function(data) {
